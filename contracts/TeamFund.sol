...
contract TeamFund is FundsRegistry {
    ...
    function TeamFund (address[] _owners, uint _signaturesRequired) public 
    ...
    modifier checkRules {
        require(closedTime != 0);
        require(devFund != 0);
        require(closedTime + 181 days <= getCurrentTime());
        _;
    }
        ...
        returns (bool)
    {
        ...
        if (closedTime + 365 days <= getCurrentTime()) {
            super.sendToken(to, value);
        } else if(closedTime + 181 days <= getCurrentTime()) {
